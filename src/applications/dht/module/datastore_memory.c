 ...
 * @param flags options for the store, MUST match the options
...
static int store(void * closure,
  ...
  while (pos != NULL) {
    if (equalsHashCode160(key, &pos->key)) {
      if (pos->flags != flags) {
	MUTEX_UNLOCK(&ds->lock);
	return DHT_ERRORCODES__INCOMPATIBLE_FLAGS;
      }
      switch (flags & DHT_FLAGS__APPEND_MODE) {
      case DHT_FLAGS__APPEND_UNIQUE:
	...
	GROW(pos->values,
	     pos->count,
	     pos->count+1);
	pos->values[pos->count-1].dataLength = value->dataLength;
	pos->values[pos->count-1].data = MALLOC(value->dataLength);
	memcpy(pos->values[pos->count-1].data,
	       value->data,
	       value->dataLength);
	MUTEX_UNLOCK(&ds->lock);  
	return OK;
	...
	return OK;
      default:
	MUTEX_UNLOCK(&ds->lock);  
	return SYSERR; /* invalid flag value */
      } /* end switch */
    } /* end key match */
    pos = pos->next;
  }
 ...
 * @param flags so far without effect
 * @return number of results, SYSERR on error
 */
