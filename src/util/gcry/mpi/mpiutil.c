...
{
  if (nlimbs <= (unsigned) a->alloced)
    return; /* no need to do it */

  if (a->d)
    a->d = realloc (a->d, nlimbs * sizeof (mpi_limb_t));
  else
    {
      if (a->flags & 1)
	/* Secure memory is wanted.  */
	a->d = calloc (nlimbs , sizeof (mpi_limb_t));
      else
	/* Standard memory.  */
	a->d = calloc (nlimbs , sizeof (mpi_limb_t));
    }
...
gcry_mpi_get_opaque( MPI a, unsigned int *nbits )
{
  if( !(a->flags & 4) ) {
    fprintf(stderr,
	    "BUG: mpi_get_opaque on normal mpi\n");
  }
    if( nbits )
	*nbits = a->sign;
    return a->d;
...
{
  char *p = gcry_random_bytes( (nbits+7)/8, level );
