	      ...
	      unsigned ivlen )
{
    ...
    if( iv ) {
      if( ivlen != BLOWFISH_BLOCKSIZE ) {
	fprintf(stderr,
		"FATAL: assert failed: cipher_setiv: ivlen=%u blklen=%u\n",
		ivlen, 
		(unsigned) BLOWFISH_BLOCKSIZE );
	exit(-1);
      }
      if (ivlen > BLOWFISH_BLOCKSIZE)
	  ivlen = BLOWFISH_BLOCKSIZE;
...
{
    byte *ivp;
    size_t blocksize = BLOWFISH_BLOCKSIZE;

    if( nbytes <= c->unused ) {
	/* short enough to be encoded by the remaining XOR mask */
	/* XOR the input with the IV and store input into IV */
	for(ivp=c->iv+BLOWFISH_BLOCKSIZE - c->unused; nbytes; nbytes--, c->unused-- )
	    *outbuf++ = (*ivp++ ^= *inbuf++);
	return;
    }
