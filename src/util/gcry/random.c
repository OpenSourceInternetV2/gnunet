...
initialize(void)
{
  ...
  if (err) {
    fprintf(stderr,
	    "FATAL: failed to create the pool lock: %s\n", strerror (err) );
    exit(-1);
  }
  ...
  rndpool = calloc(1,POOLSIZE+BLOCKLEN);
  keypool = calloc(1,POOLSIZE+BLOCKLEN);
  is_initialized = 1;
...
get_random_bytes( size_t nbytes, int level, int secure )
{
    ...
    if (err) {
      fprintf(stderr,
	      "FATAL: failed to release the pool lock: %s\n", strerror (err));
      exit(-1);
    }
    return buf;
}
...
gcry_randomize (byte *buffer, size_t length, enum gcry_random_level level)
{
  ...
  if (err) {
    fprintf(stderr,
	    "FATAL: failed to acquire the pool lock: %s\n", strerror (err));
    exit(-1);
  }
  pool_is_locked = 1;
  if (level == 1)
...
#  error must have a digest length of 20 for ripe-md-160
    ...
    if ( (char*)pool == rndpool)
      {
        gcry_rmd160_hash_buffer (failsafe_digest, pool, POOLSIZE);
...
read_seed_file()
{
	...
	fprintf(stderr, _("`%s' is not a regular file - ignored\n"), seed_file_name );
	close(fd);
	return 0;
    }
    if( !sb.st_size ) {
	fprintf(stderr, _("note: random_seed file is empty\n") );
	close(fd);
	allow_seed_file_update = 1;
	return 0;
    }
    if( sb.st_size != POOLSIZE ) {
	fprintf(stderr, _("warning: invalid size of random_seed file - not used\n") );
	close(fd);
	return 0;
    }
    do {
...
_gcry_update_random_seed_file()
{
  ...
  if ( !allow_seed_file_update )
    {
      fprintf(stderr, _("note: random_seed file not updated\n"));
      return;
    }

  err = ath_mutex_lock (&pool_lock);
  if (err)
    fprintf(stderr, "failed to acquire the pool lock: %s\n", strerror (err));
  pool_is_locked = 1;
  ...
  if (fd == -1 )
    ...
    if (close(fd))
      fprintf(stderr, _("can't close `%s': %s\n"), seed_file_name, strerror(errno) );
    }

  pool_is_locked = 0;
  err = ath_mutex_unlock (&pool_lock);
  if (err)
    fprintf(stderr, "failed to release the pool lock: %s\n", strerror (err));
}
...
read_pool( byte *buffer, size_t length, int level )
{
    ...
    if( length > POOLSIZE ) {
      fprintf(stderr,
	      "BUG: too many random bits requested\n");      
    }
    ...
    if( level == 2 && pool_balance < (int)length ) {
	size_t needed;
...
# endif
  ...
  fprintf(stderr,
	  "ERROR: no entropy gathering module detected\n");
...
read_random_source( int requester, size_t length, int level )
{
	...
	fprintf(stderr, "No way to gather entropy for the RNG\n");
}
	      ...
	      size_t length, int level )
{
    ...
    if( !initialized ) {
	fprintf(stderr, _("WARNING: using insecure random number generator!!\n"));
